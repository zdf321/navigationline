...
public class WebUtils {
  ...
  /**
   * {@link org.apache.shiro.session.Session Session} key used to save a request and later restore
   * it, for example when redirecting to a requested page after login, equal to {@code
   * shiroSavedRequest}.
   */
   ...
   *
   * <p>If included via a RequestDispatcher, the current resource will see the originating request.
   ...
   *
   * <p>If forwarded to via a RequestDispatcher, the current resource will see its own URI and
   * paths. The originating URI and paths are exposed as request attributes.
   */
  ...
  /**
   * Default character encoding to use when <code>request.getCharacterEncoding</code> returns <code>
   * null</code>, according to the Servlet spec.
   *
   ...
   *
   * <p>For example, for a request to URL
   *
   * <p><code>http://www.somehost.com/myapp/my/url.jsp</code>,
   *
   * <p>for an application deployed to <code>/mayapp</code> (the application's context path), this
   * method would return
   *
   * <p><code>/my/url.jsp</code>.
   *
   ...
   *
   * <p>As the value returned by <code>request.getRequestURI()</code> is <i>not</i> decoded by the
   ...
   * <strong>WARNING</strong> - This method is useful only for normalizing application-generated
   * paths. It does not try to perform security checks for malicious input. Normalize operations
   * were was happily taken from org.apache.catalina.util.RequestUtil in Tomcat trunk, r939305
   *
   ...
   *
   * <p>As the value returned by <code>request.getContextPath()</code> is <i>not</i> decoded by the
   * servlet container, this method will decode it.
   *
   * @param request current HTTP request
   ...
   *
   * <p>The default implementation checks the request's {@link ServletRequest#getCharacterEncoding()
   * character encoding}, and if that <code>null</code>, falls back to the {@link
  ...
  /**
   * A convenience method that merely casts the incoming <code>ServletRequest</code> to an <code>
   * HttpServletRequest</code>:
   *
   * <p><code>return (HttpServletRequest)request;</code>
   *
   * <p>Logic could be changed in the future for logging or throwing an meaningful exception in non
  ...
  /**
   * A convenience method that merely casts the incoming <code>ServletResponse</code> to an <code>
  ...
  /**
   * Checks to see if a request param is considered true using a loose matching strategy for general
   * values that indicate that something is true or enabled, etc.
   *
   * <p>
   *
   * <p>Values that are considered "true" include (case-insensitive): true, t, 1, enabled, y, yes,
   * on.
   *
  ...
  /**
   * Convenience method that returns a request parameter value, first running it through {@link
